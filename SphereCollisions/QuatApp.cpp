...
private:
	...
	//List of all sphere render items
	std::vector<RenderItem*> mSphereRitems;

	//List of all spheres items world matrix as float4x4
	std::vector<XMFLOAT4X4> mSphereRWorlds;
	...
	//create a sphere render item pointer and world location
	RenderItem* mSphereRitem;
	XMFLOAT4X4 mSphereWorld = MathHelper::Identity4x4();

	//create a second sphere
	RenderItem* mSphereRitem2;
	//XMFLOAT4X4 mSphereWorld2 = MathHelper::Identity4x4();

	//functions for sphere movement and collision
	void MoveSpheres();
	void CheckSphereCollsions();
	void HandleSphereCollisions(RenderItem* a, RenderItem* b);

	//Method to initialize the velocities of the spheres
	void InitializeSphereVelocities();
...
//Initalize each Spheres velocity vector to point towards the center of the room 
void QuatApp::InitializeSphereVelocities()
{
	float speedIncrement = 0;
	for (int i = 0; i < mSphereRitems.size(); i++)
	{
		speedIncrement += 2.0f;
		XMVECTOR ballToCenter = -XMVectorSet(mSphereRitems[i]->World._41 - 0.0f, mSphereRitems[i]->World._42 - 0.0f, mSphereRitems[i]->World._43 - 0.0f, mSphereRitems[i]->World._44);
		mSphereRitems[i]->Velocity = XMVector4Normalize(ballToCenter) * (0.001f  * speedIncrement);
	}
}

void QuatApp::MoveSpheres()
{
	for (int i = 0; i < mSphereRitems.size(); i++)
	{
		//mSphereRitems[i]->World = mSphereRWorlds[i];
		/*XMVECTOR ballToCenter = -XMVectorSet(mSphereRitems[i]->World._41 - 0.0f, mSphereRitems[i]->World._42 - 0.0f, mSphereRitems[i]->World._43 - 0.0f, mSphereRitems[i]->World._44);
		mSphereRitems[i]->Velocity = XMVector4Normalize(ballToCenter) * (0.001f  * i);*/

		XMMATRIX translationMat = XMMatrixTranslationFromVector(mSphereRitems[i]->Velocity);

		XMMATRIX currPos = XMLoadFloat4x4(&mSphereRitems[i]->World);

		XMMATRIX result = XMMatrixMultiply(currPos, translationMat);

		XMStoreFloat4x4(&mSphereWorld, result);

		mSphereRitems[i]->World = mSphereWorld;
		mSphereRitems[i]->NumFramesDirty = gNumFrameResources;
	
	}
}

//checks a collision between mSphereRitem and mSphereRitem2
void QuatApp::CheckSphereCollsions()
{
	for (int i = 0; i < mSphereRitems.size(); i++)
	{
		
		for (int j = 0; j < mSphereRitems.size(); j++)
		{
			if (i != j) // make sure we are not check a shere against itself
			{
				XMVECTOR item1Pos = XMVectorSet(mSphereRitems[i]->World._41, mSphereRitems[i]->World._42, mSphereRitems[i]->World._43, mSphereRitems[i]->World._44);
				XMVECTOR item2Pos = XMVectorSet(mSphereRitems[j]->World._41, mSphereRitems[j]->World._42, mSphereRitems[j]->World._43, mSphereRitems[j]->World._44);

				//get the distance bweteen the two spheres World positions
				XMVECTOR distanceVec = item2Pos - item1Pos;
				XMVECTOR dis = XMQuaternionLengthSq(distanceVec); 
				XMFLOAT4 dist;
				XMStoreFloat4(&dist, dis);

				if (dist.w <= 1.0f) // tweek this number for the radii of the spheres
				{
					HandleSphereCollisions(mSphereRitems[i], mSphereRitems[j]);
				}
			}
		}
	}
...
//Handles a collision between mSphereRitem and mSphereRitem2
void QuatApp::HandleSphereCollisions(RenderItem* a, RenderItem* b)
{
	float scalar = 0.001f;
	//change the spheres material on collision
	a->Mat = mMaterials["tile0"].get();
	b->Mat = mMaterials["tile0"].get();

	XMVECTOR vecAB = XMVectorSet(a->World._41 - b->World._41, a->World._42 - b->World._42, a->World._43 - b->World._43, a->World._44 - b->World._44);
	XMVECTOR vecBA = XMVectorSet(b->World._41 - a->World._41, b->World._42 - a->World._42, b->World._43 - a->World._43, b->World._44 - a->World._44);

	a->Velocity = vecAB * scalar;
	b->Velocity = vecBA * scalar;
}
...
void QuatApp::BuildMaterials()
{
	...
	stone0->DiffuseAlbedo = XMFLOAT4(0.0f, 1.0f, 0.0f, 1.0f);
    bricks0->FresnelR0 = XMFLOAT3(0.1f, 0.9f, 0.1f);
...
void QuatApp::BuildRenderItems()
{
   /* auto skullRitem = std::make_unique<RenderItem>();
    XMStoreFloat4x4(&skullRitem->World,
        XMMatrixScaling(0.25f, 0.25f, 0.25f)*XMMatrixTranslation(0.0f, 1.0f, 0.0f));
	...
	XMStoreFloat4x4(&boxRitem->World, XMMatrixScaling(1.5f, 0.5f, 1.5f)*XMMatrixTranslation(0.0f, 0.5f, 0.0f));
   ...
   // gridRitem->World = MathHelper::Identity4x4() * XMMatrixTranslation(0.0f, -0.5f, 0.0f);
	...
	for(int i = 0; i < 10; ++i)
	{
		...
		XMStoreFloat4x4(&mSphereWorld, leftSphereWorld);
		mSphereRWorlds.push_back(mSphereWorld);

		XMStoreFloat4x4(&mSphereWorld, rightSphereWorld);
		mSphereRWorlds.push_back(mSphereWorld);
		...
		//make this the mSphereRitem
		mSphereRitem = leftSphereRitem.get();
		mSphereRitems.push_back(mSphereRitem);
		...
		mSphereRitem2 = rightSphereRitem.get();
		mSphereRitems.push_back(mSphereRitem2);

		//add all sphere items to the list of R item pointers
		//mSphereRitems.push_back(std::move(leftSphereRitem));
		//mSphereRitems.push_back(std::move(rightSphereRitem));
		...
		//set the inital velocity of each sphere to be towards the center of the world
		InitializeSphereVelocities();

		//add all sphere item world matrices to list of matrices
		//mSphereRWorlds.push_back(std::move(leftSphereRitem->World));
		//mSphereRWorlds.push_back(std::move(rightSphereRitem->World));
	}
